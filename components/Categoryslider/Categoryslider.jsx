...
const CarouselInScrollContainer = (props) => {
  ...
  const filteredCards =
    selectedCategory === "all"
      ? cards
      : cards.filter((card) => card.catslug === selectedCategory);
  ...
  return (
    <div>
          ...
          <div className={styles.scrollingcontainer} ref={containerRef}>
            ...
            {sortedCategories.map((buttonlabel) => (
              <button
                type="button"
                key={buttonlabel.id}
                onClick={() => handleButtonClick(buttonlabel.slug)}
                className={
                  buttonlabel.slug === selectedCategory
                    ? styles.activebutton
                    : styles.nonactivebutton
                }
              >
      ...
      <Cardlist cardsArray={filteredCards} resetpage={1} />
    </div>
  );
};
